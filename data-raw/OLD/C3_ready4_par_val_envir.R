...
setOldClass(c("ready4_param_val_envir","tbl_df", "tbl", "data.frame"))
...
#' @param x A prototype for the Readyforwhatsnext S3 class for tibble object that stores simulation parameter values for each iteration., Default: make_pt_ready4_param_val_envir()
...
#' @rdname ready4_param_val_envir
#' @export

ready4_param_val_envir <- function(x = make_pt_ready4_param_val_envir()){
validate_ready4_param_val_envir(make_new_ready4_param_val_envir(x))
}
...
#' @details Readyforwhatsnext S3 class for tibble object that stores simulation parameter values for each iteration.
#' @rdname make_new_ready4_param_val_envir
#' @export
#' @importFrom tibble is_tibble
make_new_ready4_param_val_envir <- function(x){
stopifnot(tibble::is_tibble(x))
class(x) <- append(c("ready4_param_val_envir",setdiff(make_pt_ready4_param_val_envir() %>% class(),class(x))),
class(x))
x
}
...
#' @details Readyforwhatsnext S3 class for tibble object that stores simulation parameter values for each iteration.
#' @rdname make_pt_ready4_param_val_envir
#' @export
#' @importFrom ready4 update_pt_fn_args_ls
#' @importFrom rlang exec
#' @importFrom tibble tibble
make_pt_ready4_param_val_envir <- function(param_name_chr = character(0),
v_it_1 = numeric(0)){
args_ls <- list(param_name_chr = param_name_chr,
v_it_1 = v_it_1) %>% ready4::update_pt_fn_args_ls()
...
#' @details Readyforwhatsnext S3 class for tibble object that stores simulation parameter values for each iteration.
#' @rdname validate_ready4_param_val_envir
#' @export
#' @importFrom stringr str_detect str_c
#' @importFrom dplyr summarise_all arrange filter pull
#' @importFrom tidyr gather
#' @importFrom purrr map2_chr
validate_ready4_param_val_envir <- function(x){
if(sum(stringr::str_detect(names(x)[names(x) %in% names(make_pt_ready4_param_val_envir())],
names(make_pt_ready4_param_val_envir())))!=length(names(make_pt_ready4_param_val_envir()))){
stop(paste0("TIBBLE must include columns named: ",
names(make_pt_ready4_param_val_envir()) %>% stringr::str_c(sep="", collapse = ", ")),
call. = FALSE)
}
 if(!identical(make_pt_ready4_param_val_envir() %>%
...
dplyr::summarise_all(class) %>%
 tidyr::gather(variable,class) %>%
dplyr::filter(variable %in% names(make_pt_ready4_param_val_envir())) %>% dplyr::arrange(variable))){
stop(paste0("TIBBLE columns should be of the following classes: ",
purrr::map2_chr(make_pt_ready4_param_val_envir() %>%
dplyr::summarise_all(class) %>%
 tidyr::gather(variable,class) %>%
dplyr::pull(1),
 make_pt_ready4_param_val_envir() %>%
dplyr::summarise_all(class) %>%
 tidyr::gather(variable,class) %>%
dplyr::pull(2),
 ~ paste0(.x,": ",.y)) %>%
...
#' @details Readyforwhatsnext S3 class for tibble object that stores simulation parameter values for each iteration.
#' @rdname is_ready4_param_val_envir
#' @export

is_ready4_param_val_envir <- function(x) inherits(validate_ready4_param_val_envir(x), "ready4_param_val_envir")
