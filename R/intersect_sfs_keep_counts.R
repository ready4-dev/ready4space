...
intersect_sfs_keep_counts <- function(profiled_sf,
  ...
  #                                       "_",
  #                                       .)))
  attribute_sf <- add_feature_areas(sf = attribute_sf,
                                    data_type = data_type,
                                    data_year = data_year,
                                    popl_var_prefix = popl_var_prefix,
                                    feature = attribute_unit)
  profiled_sf <- intersect_lon_lat_sfs(sf_1 = profiled_sf,
                                       sf_2 = attribute_sf,
                                       crs_nbr_vec = crs_nbr_vec) #sf::st_intersection(profiled_sf, attribute_sf)
  profiled_sf <- profiled_sf %>%
...
#' }
#' @seealso
#'  \code{\link[dplyr]{mutate}},\code{\link[dplyr]{select_all}},\code{\link[dplyr]{vars}},\code{\link[dplyr]{funs}}
#'  \code{\link[rlang]{sym}}
#'  \code{\link[sf]{geos_measures}}
#'  \code{\link[units]{set_units}}
#' @rdname add_feature_areas
#' @export
#' @importFrom dplyr mutate rename_at vars funs
#' @importFrom rlang sym
#' @importFrom sf st_area
#' @importFrom units set_units
add_feature_areas <- function(sf,
                              data_type,
                              data_year,
                              feature,
                              popl_var_prefix = NULL){
