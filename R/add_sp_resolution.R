...
#' }
#' @seealso
#'  \code{\link[ready4s4]{sp_data_pack_lup}},\code{\link[ready4s4]{sp_resolution_lup<-}}
...
#'  \code{\link[ready4s3]{ready4_sp_resolution_lup}}
#' @rdname add_sp_resolution
#' @export
#' @importFrom ready4s4 sp_data_pack_lup sp_resolution_lup<-
#' @importFrom dplyr filter select mutate pull arrange
#' @importFrom stringr str_which
#' @importFrom purrr pmap_dfr
#' @importFrom tibble tibble
#' @importFrom ready4s3 ready4_sp_resolution_lup
add_sp_resolution <- function(lookup_tbs_r4,
                              processed_dir){
  dr_dp_tb <- ready4s4::sp_data_pack_lup(lookup_tbs_r4) %>%
  ...
  resolution_lup_r3 <- resolution_lup_r3 %>%
    ready4s3::ready4_sp_resolution_lup() %>%
    dplyr::arrange(mean_size)
  ready4s4::`sp_resolution_lup<-`(lookup_tbs_r4, resolution_lup_r3)
}
