...
setOldClass(c("ready4_par_str_envir","tbl_df", "tbl", "data.frame"))
...
#' @param x A prototype for the Readyforwhatsnext S3 class for tibble object that stores simulation structural parameters relating to the spatial environment., Default: make_pt_ready4_par_str_envir()
...
ready4_par_str_envir <- function(x = make_pt_ready4_par_str_envir()){ 
validate_ready4_par_str_envir(make_new_ready4_par_str_envir(x))
}
...
#' @details Readyforwhatsnext S3 class for tibble object that stores simulation structural parameters relating to the spatial environment.
#' @rdname make_new_ready4_par_str_envir
#' @export 
#' @importFrom tibble is_tibble
make_new_ready4_par_str_envir <- function(x){ 
stopifnot(tibble::is_tibble(x))
class(x) <- append(c("ready4_par_str_envir",setdiff(make_pt_ready4_par_str_envir() %>% class(),class(x))),
class(x))
x
}
...
#' @details Readyforwhatsnext S3 class for tibble object that stores simulation structural parameters relating to the spatial environment.
#' @rdname make_pt_ready4_par_str_envir
#' @export 
...
#' @details Readyforwhatsnext S3 class for tibble object that stores simulation structural parameters relating to the spatial environment.
#' @rdname validate_ready4_par_str_envir
#' @export 
...
if(sum(stringr::str_detect(names(x)[names(x) %in% names(make_pt_ready4_par_str_envir())],
names(make_pt_ready4_par_str_envir())))!=length(names(make_pt_ready4_par_str_envir()))){
stop(paste0("TIBBLE must include columns named: ",
names(make_pt_ready4_par_str_envir()) %>% stringr::str_c(sep="", collapse = ", ")),
call. = FALSE)
}
 if(!identical(make_pt_ready4_par_str_envir() %>% 
...
dplyr::summarise_all(class) %>% 
 tidyr::gather(variable,class) %>% 
dplyr::filter(variable %in% names(make_pt_ready4_par_str_envir())) %>% dplyr::arrange(variable))){
stop(paste0("TIBBLE columns should be of the following classes: ",
purrr::map2_chr(make_pt_ready4_par_str_envir() %>% 
dplyr::summarise_all(class) %>% 
 tidyr::gather(variable,class) %>% 
dplyr::pull(1),
 make_pt_ready4_par_str_envir() %>% 
dplyr::summarise_all(class) %>% 
 tidyr::gather(variable,class) %>% 
dplyr::pull(2),
 ~ paste0(.x,": ",.y)) %>% 
...
#' @details Readyforwhatsnext S3 class for tibble object that stores simulation structural parameters relating to the spatial environment.
#' @rdname is_ready4_par_str_envir
#' @export 
